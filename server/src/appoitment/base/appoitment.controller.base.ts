...
export class AppoitmentControllerBase {
  ...
  async findManyService(
    ...
    const results = await this.service.findService(params.id, {
      where: query,
      select: {
        createdAt: true,
        id: true,
  ...
  async createService(
    @common.Param() params: AppoitmentWhereUniqueInput,
    @common.Body() body: AppoitmentWhereUniqueInput[],
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<void> {
    const data = {
      service: {
    ...
    if (invalidAttributes.length) {
      ...
      throw new common.ForbiddenException(
        `Updating the relationship: ${
          invalidAttributes[0]
        } of ${"Appoitment"} is forbidden for roles: ${roles}`
      );
    }
