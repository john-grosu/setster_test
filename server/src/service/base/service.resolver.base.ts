...
import { AppoitmentFindManyArgs } from "../../appoitment/base/AppoitmentFindManyArgs";
...
export class ServiceResolverBase {
  ...
  @graphql.ResolveField(() => [Appoitment])
  @nestAccessControl.UseRoles({
    resource: "Service",
    action: "read",
    possession: "any",
  ...
  async appoitments(
    ...
    const results = await this.service.findAppoitments(parent.id, args);
    return results.map((result) => permission.filter(result));
  }
}
